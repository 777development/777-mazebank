...
RegisterNetEvent('thermite:StartFire', function(coords, maxChildren, isGasFire)
    if #(vector3(coords.x, coords.y, coords.z) - GetObjectCoords(PlayerPedId())) < 100 then
        local pos = {
            x = coords.x,
            y = coords.y,
            z = coords.z,
...
RegisterNetEvent('777-mazebank:client:SetStationStatus', function(key, isHit)
...
RegisterNUICallback('thermitefailed', function(_, cb)
    QBCore.Functions.TriggerCallback("thermite:server:check", function(success)
        if success then
            ...
            local coords = GetObjectCoords(PlayerPedId())
    ...
    QBCore.Functions.TriggerCallback("thermite:server:check", function(success)
        if success then
            ...
            if currentStation ~= 0 then
                ...
                TriggerServerEvent("777-mazebank:server:SetStationStatus", currentStation, true)
